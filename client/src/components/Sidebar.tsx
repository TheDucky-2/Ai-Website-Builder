...
import type { SidebarProps, Message, Version } from "../types/index.ts"
...
const Sidebar = ({isMenuOpen, project, setProject, isGenerating, setIsGenerating}: SidebarProps) => {
  ...
  const handleRevisions = async(e: SubmitEvent) => {
    e.preventDefault()
    ...
    ${isMenuOpen ? "max-sm:w-0 overflow-hidden" : "w-full" }`}>
                ...
                .sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).map((message)=> {
                    ...
                    if(isMessage){
                        ...
                        return (
                            ...
                            <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 loading-relaxed
        ...
        <form onSubmit={handleRevisions} className="m-3 relative">
            <div className="flex items-center gap-2">
                <textarea rows={4} placeholder="Describe your website or request changes..." onChange={(e) => setInput(e.target.value)} value = {input}
                ...
                className="absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600
